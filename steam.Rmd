...
Primero vamos a analizar la variable `nJuegos` que representa el numero de juegos que tiene cada usuario para ver si hay difierencias en el numero de juegos entre los usuarios normales y los que son empleados de *Valve*.
```{r valve-order}
# crear un nuevo data.frame ordenado por el numero de juegos
valve.order <- valve[order(valve$nJuegos),]
```

Visualizamos el numero de juegos.
...
Se puede ver que los que son empleados de *Valve* tienden a tener más juegos. Explicación^[https://www.reddit.com/r/Steam/comments/2qtrrx/so_you_think_gabe_newell_can_play_any_game_he/cn9hvfl/] no es oficial?

Histograma de numero de juegos bins de 20 juegos de ancho, el ultimo bin representa el numero de usuarios con mas de 800 juegos.
...
La siguiente figura representa el conteo total de etiquetas por genero ordenado de mayor a menor.
...
Quitar outliers, considerar outliers a jugadores con mas de 1000 juegos
...
Boxplots nJuegos, Singleplayer y Multiplayer.
...
Funcion para comparar dos usuarios en un radarchart
